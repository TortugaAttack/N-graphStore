 ...
 *
 */
public class AddMergerTest {

	@Test
	public void emptyTest() {
		SimpleResultSet res = new SimpleResultSet();
		AddMerger addMerger = new AddMerger();
		SimpleResultSet join = addMerger.merge(res, res);
	...
	public void oneEmptyTest() {
		...
		AddMerger addMerger = new AddMerger();
		SimpleResultSet join = addMerger.merge(empty, res);
		...
		join = addMerger.merge(res, empty);
	...
	public void checkJoin() {
		...
		SimpleResultSet join = addMerger.merge(res1, res2);
		assertTrue(join.getVars().size() == 3);
		assertTrue(join.getVars().contains("s"));
		assertTrue(join.getVars().contains("p"));
		assertTrue(join.getVars().contains("o"));

		assertTrue(join.getRows().size() == 5);
		List<String> expected = new LinkedList<String>();
		expected.add("<urn://test2>");
		expected.add(null);
		expected.add("<urn://test>");
